...
// ═══════ Available Schedule Check ═══════
async function checkSchedule() {
  try {
    const ecUrl = process.env.EDGE_CONFIG;
    if (!ecUrl) return null;
    const resp = await fetch(`${ecUrl.split('?')[0]}/items?${ecUrl.split('?')[1]}`);
    if (resp.ok) {
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      // Log inquiry
      const all = await readInquiries();
      const existingSession = all.find(s => s.sessionId === sessionId);
      ...
      if (existingSession) {
        ...
        existingSession.category = result.category;
